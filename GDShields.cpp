...
#include "GDShields.h"
  ...
  #include "OpenSmart_Serial_Shields.h"
#else
	#include "OpenSmart_Parallel_Shields.h"
#endif
...
Point::Point(int16_t x0, int16_t y0) { x = x0; y = y0; }
bool Point::operator==(Point p) { return  ( (p.x == x) && (p.y == y) ); }
bool Point::operator!=(Point p) { return  ( (p.x != x) || (p.y != y) ); }
...
bool TSPoint::operator==(TSPoint p) { return  ((p.x == x) && (p.y == y) && (p.z == z)); }
bool TSPoint::operator!=(TSPoint p) { return  ((p.x != x) || (p.y != y) || (p.z != z)); }

TouchScreen::TouchScreen(	bool _Calibrated, uint16_t _PT,	
													double _kx1, double _kx2, double _kx3, 
  												double _ky1, double _ky2, double _ky3)
  						{
  							Calibrated = _Calibrated;
  							PT = _PT;
  							kx1 = _kx1; kx2 = _kx2; kx3 = _kx3;
  							ky1 = _ky1; ky2 = _ky2; ky3 = _ky3;
  						}
void TouchScreen::setCalibrated(bool _Calibrated) { Calibrated = _Calibrated; }
void TouchScreen::setPressureThreshhold(uint16_t _PT) { PT = _PT; }
void TouchScreen::setCalibrationConstants( double _kx1, double _kx2, double _kx3, double _ky1, double _ky2, double _ky3)
							{
  							kx1 = _kx1; kx2 = _kx2; kx3 = _kx3;
  							ky1 = _ky1; ky2 = _ky2; ky3 = _ky3;
							}
...
TSPoint TouchScreen::getPoint(void) {
    ...
    float diff;
		...
		while (z <= PT ) 
		{
	    pinMode(TOUCHSCREEN_YP, INPUT);
	    pinMode(TOUCHSCREEN_YM, INPUT);
	    digitalWrite(TOUCHSCREEN_YP, LOW);
	    digitalWrite(TOUCHSCREEN_YM, LOW);

	    pinMode(TOUCHSCREEN_XP, OUTPUT);
	    pinMode(TOUCHSCREEN_XM, OUTPUT);
	    digitalWrite(TOUCHSCREEN_XP, HIGH);
	    digitalWrite(TOUCHSCREEN_XM, LOW);

	    while (1) {
	      a1 = analogRead(TOUCHSCREEN_YP);
	      a2 = analogRead(TOUCHSCREEN_YP);
	      a3 = analogRead(TOUCHSCREEN_YP);
	      a4 = analogRead(TOUCHSCREEN_YP);
	      diff = (abs(a1 - a2) + abs(a2 - a3) + abs(a3 - a4)) / 3;
	      x = (a1 + a2 + a3 + a4) / 4;
	      if (diff < (0.02*x)) break;
	    }

	    pinMode(TOUCHSCREEN_XP, INPUT);
	    pinMode(TOUCHSCREEN_XM, INPUT);
	    digitalWrite(TOUCHSCREEN_XP, LOW);
	    digitalWrite(TOUCHSCREEN_XM, LOW);     //.kbv for Due

	    pinMode(TOUCHSCREEN_YP, OUTPUT);
	    pinMode(TOUCHSCREEN_YM, OUTPUT);
	    digitalWrite(TOUCHSCREEN_YP, HIGH);
	    digitalWrite(TOUCHSCREEN_YM, LOW);     //.kbv for Due

	    while (1) {
	      a1 = analogRead(TOUCHSCREEN_XM);
	      a2 = analogRead(TOUCHSCREEN_XM);
	      a3 = analogRead(TOUCHSCREEN_XM);
	      a4 = analogRead(TOUCHSCREEN_XM);
	      diff = (abs(a1 - a2) + abs(a2 - a3) + abs(a3 - a4)) / 3;
	      y = (a1 + a2 + a3 + a4) / 4;
	      if (diff < (0.02*x)) break;
	    }
	    ...
	    // Set Y- to VCC
	    pinMode(TOUCHSCREEN_YM, OUTPUT);     //.kbv
	    digitalWrite(TOUCHSCREEN_YM, HIGH);

	    // Hi-Z X- and Y+
	    digitalWrite(TOUCHSCREEN_XM, LOW);   //.kbv
	    pinMode(TOUCHSCREEN_XM, INPUT);      //.kbv
	    digitalWrite(TOUCHSCREEN_YP, LOW);
	    pinMode(TOUCHSCREEN_YP, INPUT);

	    int z1 = ( analogRead(TOUCHSCREEN_XM)+analogRead(TOUCHSCREEN_XM) )/2;
	    int z2 = ( analogRead(TOUCHSCREEN_YP)+analogRead(TOUCHSCREEN_YP) )/2;
    ...
    if ( Calibrated ) {
    	x = trunc(kx1*x + kx2*y + kx3);
  		y = trunc(ky1*x + ky2*y + ky3);
  	}
...
bool TouchScreen::isTouching(void)
{
	...
	return ( ((1023 - (z2 - z1)) <= PT) ? false : true); 
}		


#endif // GDS_CAPABILITIES_TOUCHSCREEN
...
GDS::GDS(int CS, int RS, int WR, int RD, int RST):Adafruit_GFX(240, 400)
{
...
void GDS::WriteCmdData(uint16_t cmd, uint16_t dat)
{
  CS_ACTIVE;
  WriteCmd(cmd);
  WriteData(dat);
...
void GDS::pushCommand(uint16_t cmd, uint8_t * block, int8_t N) { WriteCmdParamN(cmd, N, block); }
...
uint16_t GDS::readReg(uint16_t reg)
{
  uint16_t ret;
  uint8_t lo;
  if (!done_reset)
...
uint32_t GDS::readReg32(uint16_t reg)
{
  uint16_t h, l;
  CS_ACTIVE;
  WriteCmd(reg);
...
uint16_t GDS::readID(void)
{
  uint16_t ret, ret2;
  uint8_t msb;

  ret = readReg(0);           			//forces a reset() if called before begin()
  if (ret == 0x5408) return 0x5408; //the SPFD5408 fails the 0xD3D3 test.
  if (ret == 0x0089 || ret == 0x8989) return 0x1289;
  ...
  if (ret == 0x6101) return 0x1963;
	...
	if (ret == 0x9481) return 0x9481; //ILI9481: [xx 02 04 94 81 FF]
  if (ret == 0x1511) return 0x1511; //?R61511: [xx 02 04 15 11] not tested yet
  if (ret == 0x1520) return 0x1520; //?R61520: [xx 01 22 15 20]
  if (ret == 0x1400) return 0x6814; //?RM68140:[xx FF 68 14 00] not tested yet
        
  ret = readReg40(0xEF);            //ILI9327: [xx 02 04 93 27 FF] 
  if (ret == 0x9327) return 0x9327;
  ret = readReg32(0x04);            //ST7789V: [85 85 52] 
  if (ret == 0x8000) return 0x8357; //HX8357-D
  if (ret == 0x8552) return 0x7789;
  ret = readReg32(0xD3);      //for ILI9488, 9486, 9340, 9341
  msb = ret >> 8;
  if (msb == 0x93 || msb == 0x94) return ret;             //0x9488, 9486, 9340, 9341
...
int16_t GDS::readGRAM(int16_t x, int16_t y, uint16_t * block, int16_t w, int16_t h)
{
...
void GDS::setRotation(uint8_t r)
{
...
void GDS::drawPixel(int16_t x, int16_t y, uint16_t color)
{
  ...
  if (x < 0 || y < 0 || x >= width() || y >= height()) return;
	#if defined(OFFSET_ILI9327)
		if (_lcd_ID == 0x9327) {
	    if (rotation == 2) y += OFFSET_ILI9327;
	    if (rotation == 3) x += OFFSET_ILI9327;
    }
	#endif
  if (_lcd_capable & MIPI_DCS_REV1) {
...
void GDS::setAddrWindow(int16_t x, int16_t y, int16_t x1, int16_t y1)
{
#if defined(OFFSET_ILI9327)
	if (_lcd_ID == 0x9327) {
	    if (rotation == 2) y += OFFSET_ILI9327, y1 += OFFSET_ILI9327;
	    if (rotation == 3) x += OFFSET_ILI9327, x1 += OFFSET_ILI9327;
    }
#endif
    if (_lcd_capable & MIPI_DCS_REV1) {
...
void GDS::fillRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color)
{
  #ifdef ILI9327_SPECIAL
    color = ~color;
	#endif
...
void GDS::pushColors(const uint16_t color, int16_t n, bool first)
{
  CS_ACTIVE;
  if (first) {
      WriteCmd(_MW);
...
void GDS::pushColors(uint16_t * block, int16_t n, bool first)
{
  uint16_t color;
...
void GDS::pushColors(uint8_t * block, int16_t n, bool first)
{
  uint16_t color;
  uint8_t h, l;
...
void GDS::pushColors(const uint8_t * block, int16_t n, bool first)
{
  uint16_t color;
  uint8_t h, l;
...
void GDS::vertScroll(int16_t top, int16_t scrollines, int16_t offset)
{
#if defined(OFFSET_ILI9327)
	if (_lcd_ID == 0x9327) {
	    if (rotation == 2 || rotation == 3) top += OFFSET_ILI9327;
    }
#endif
...
void GDS::invertDisplay(boolean i)
{
    uint8_t val;
...
void GDS::begin(uint16_t ID)
{
    ...
    case 0x1520:
        ...
        break;
#endif
#if defined(SUPPORT_SS1963) && USING_16BIT_BUS 
	case 0x1963:
    ...
    case 0x7789:
        ...
        break;
#endif        
#if defined(SUPPORT_HX8347D) || defined(SUPPORT_HX8347G) || defined(SUPPORT_HX8347I)
...
#define GAMMA9486 4
    ...
    case 0x9488:
        ...
        *p16 = 320;
        break;
#endif        
#if defined(SUPPORT_R61303V) || defined(SUPPORT_R61505W)
    case 0xB505:                //R61505V
	  case 0xC505:                //R61505W
	  case 0x505:
